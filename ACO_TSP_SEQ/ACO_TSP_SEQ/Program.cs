...
(double[,] result, double tempoExecucao) MedirTempoExecucao(Action<double[,], double[,], double[,], int, int, int> func, double[,] distancias, double[,] feromonios, double[,] heuristicas, int numCidades, int numFormigas, int numIteracoes)
{
...
using var file = new StreamWriter("C:\\Users\\dougl\\source\\repos\\ACO\\tempo_execucao_sequencial.txt");

foreach (var numIteracoes in iteracoesArray)
{
    var (result, tempoExecucao) = MedirTempoExecucao(Aco, distancias, feromonios, heuristicas, numCidades, numFormigas, numIteracoes);
